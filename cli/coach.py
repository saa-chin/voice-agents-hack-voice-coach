...
# Breathing room AFTER the coach finishes speaking feedback, BEFORE we
# speak the next drill prompt. Without this, a clinician-style session
# feels rushed: feedback ends and the next "Please say: X" starts
# immediately, so the patient has no moment to process the cue. Env var
# VOICE_COACH_POST_FEEDBACK_PAUSE_MS lets clinicians dial it up for
# patients with slower processing.
...
POST_FEEDBACK_PAUSE_MS = _env_int("VOICE_COACH_POST_FEEDBACK_PAUSE_MS", 1500)
...
4. WRITE ack + feedback to MATCH next_action. NEVER praise a mismatch.
   Feedback must be SPECIFIC — name how the patient is doing on the
   vocal dimensions, don't just say "good" or "try again". Think like
   a speech therapist giving a short debrief:
   ...
   - matched_prompt = false:
     ...
     * "feedback" must redirect their DELIVERY without repeating,
       quoting, or scripting the target utterance. The system will
       re-speak the prompt for them; your job is to reset how they
       produce the sound — breath support, posture, outside voice,
       focus. NEVER write "say X" or "try saying X" or quote the
       target word back to them.
       Good: "Let's reset — take a deep breath and bring your outside
       voice when you hear the cue again."
       Good: "Pause, anchor on your breath, and let the sound carry
       from the chest this time."
       BAD : "The prompt was 'Help' — say it again."  (restates target)
       BAD : "Try saying 'Help' with more force."     (scripts content)
     * Do NOT narrate loudness / pitch / pace / articulation on a
       mismatch — they said the wrong thing, there is nothing useful
       to grade yet.

   - matched_prompt = true (advance OR retry): the attempt was the
     right utterance, so GRADE and REPORT on how they produced it.
     Your feedback MUST:
     (a) open with a brief, genuine encouragement ("Nice work", "Good
         effort", "You're getting there") — ONE short phrase, no
         over-the-top praise;
     (b) reference AT LEAST TWO of the four vocal dimensions —
         loudness, pitch range, pace, articulation — using the
         metrics_observed booleans as ground truth. Describe the
         dimension in plain, patient-friendly language:
           loudness_ok=true   → "your volume landed on target",
                                "strong, full voice"
           loudness_ok=false  → "volume was a bit soft", "try to push
                                 the sound out further"
           pitch_range_ok=t   → "nice variation in your pitch"
           pitch_range_ok=f   → "it came out fairly monotone — add a
                                 little rise and fall"
           pace_ok=true       → "pacing was steady"
           pace_ok=false      → "that was a touch rushed, slow it down
                                 a hair" / "it trailed a bit, keep the
                                 energy through the end"
           articulation_ok=t  → "your words were crisp and clear"
           articulation_ok=f  → "the consonants blurred a little —
                                 exaggerate the sounds"
     (c) if next_action = "retry", end with ONE concrete DELIVERY cue
         — what to EMPHASIZE or shape differently about HOW they
         produce the sound (breath support, consonant crispness,
         vowel length, pitch rise, stress, pace, projection). NEVER
         tell them what to say or repeat the target word back at
         them; only coach the production.
         Good: "Lean into the final consonant and hold the vowel a
                beat longer."
         Good: "Push from the belly and keep the pitch bright all
                the way through."
         BAD : "Try saying 'Help' again with more volume."  (scripts
                                                             content)
         If next_action = "advance", end with a short forward-looking
         line ("same energy on the next one", "keep that going").
...
Worked examples for "Expected utterance: Help":
  ...
  Input: clear "Help" at target loudness, full articulation
    -> {{"heard":"help","ack":"Clear and strong.",
         "feedback":"Nice work — your volume landed right on target and the word came out crisp. Pitch had good weight too. Keep that same energy on the next one.",
  ...
  Input: correct word but soft and a touch mumbled
    -> {{"heard":"help","ack":"Right word.",
         "feedback":"Good effort — your pacing was steady and you kept the pitch up, but the volume was softer than target and the consonants blurred a little. One more time with a fuller breath and a sharper 'p' at the end.",
         "next_action":"retry","metrics_observed":{{"matched_prompt":true,"loudness_ok":false,
         "pitch_range_ok":true,"pace_ok":true,"articulation_ok":false}}}}

  Input: patient says "thank you"
    -> {{"heard":"thank you","ack":"I heard 'thank you'.",
         "feedback":"Let's reset — take a deep breath and bring your outside voice when the cue comes around again.",
  ...
  Input: patient mumbles "uhh"
    -> {{"heard":"uhh","ack":"Got a hesitation.",
         "feedback":"Pause, anchor on your breath, and let the sound carry from the chest when you hear the cue next.",
         "next_action":"retry","metrics_observed":{{"matched_prompt":false,"loudness_ok":false,
         "pitch_range_ok":false,"pace_ok":false,"articulation_ok":false}}}}
...
{{
  ...
  "ack": "<short, honest acknowledgement, 2-8 words>",
  "feedback": "<specific debrief, 18-45 words. If matched_prompt=true: open with brief encouragement, name at least two of loudness/pitch/pace/articulation using the metrics booleans, then close with one DELIVERY cue — how to shape the next attempt (breath, emphasis, pitch, pace, projection). NEVER tell them what to say or quote the target word. If matched_prompt=false: neutrally acknowledge what you heard and redirect their delivery (breath, outside voice, focus) WITHOUT quoting, restating, or scripting the target utterance.>",
...
def coach_mode(  # pragma: no cover - full hardware+model integration; covered by test_server.py via WebSocket
    ...
    try:
        ...
        while i < len(drills) and not interrupted and not rest_called:
            ...
            # Breathing room: give the patient a beat to process the
            # feedback before we launch into the next prompt. Skipped
            # on rest (we're leaving) and interruptible by Ctrl+C.
            if POST_FEEDBACK_PAUSE_MS > 0 and not interrupted:
                time.sleep(POST_FEEDBACK_PAUSE_MS / 1000.0)
